...
BEGIN
    ...
    v_search := 
        ...
        setweight(to_tsvector('english', COALESCE(NEW.tag_brand, '')), 'B') ||
...
-- 5. Populate search_vector for all existing rows using direct UPDATE
UPDATE the_vault SET search_vector = 
    ...
    setweight(to_tsvector('english', COALESCE(tag_brand, '')), 'B') ||
